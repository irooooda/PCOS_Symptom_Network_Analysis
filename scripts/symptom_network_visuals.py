...
def create_interactive_network(G, filename):
    ...
    # WORKAROUND: pyvis write_html has a bug — it ignores full paths unless cwd is set correctly
    temp_cwd = os.getcwd()
    try:
        os.chdir(full_path.parent)  # temporarily change directory
        ...
        """)
        net.write_html(full_path.name)  # just the filename, cwd is changed
